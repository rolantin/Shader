...
Shader "Rolan/t4m" {
    Properties {
        ...
        _GlossPower("雪高光范围", Range(0, 10)) = 0.37 
...
/// Final Color:
                float3 finalColor = AmbientColor * Light * atten * lerp (_blenderColor ,diff,1-_blenderIntensity);
       ...
       Pass {
           ...
           // Blend One One                      
            ...
            float4 frag(VertexOutput i) : COLOR {         
                ...
                float3x3 tangentTransform = float3x3( i.tangentDir, i.bitangentDir, i.normalDir);
                float3 viewDirection = normalize(_WorldSpaceCameraPos.xyz - i.posWorld.xyz);
                float3 _noise_var = tex2D(_Noise,TRANSFORM_TEX(i.uv0, _Noise));
