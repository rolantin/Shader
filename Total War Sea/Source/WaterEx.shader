		...
		Pass
		{
			...
			v2f vert (appdata v)
			{
			 ...
			 //去掉波浪幅度控制
			  ...
			  _dis.xy = (((_dis.xy + (    _Time.xx)) * _ReflectionTex_ST.xy) + _ReflectionTex_ST.zw);
			...
			fixed4 frag (v2f i) : SV_Target
			{
				...
				((_normal1 * _Wave1.z) + (_normal4 * _Wave2.z))+ (_normal2 * _Wave3.z)) + (_normal3 * _Wave4.z)));
               ...
               float sp_ware = pow (  clamp (  dot (_normal_final.xyz, tmpvar_18) , 0.0, 1.0) ,  _SparklingSpecularPower);
				...
				float tmpvar_25 = tex2D (_MaskMap, P_24).x;
				maskAlpha_3 = tmpvar_25;
				alpha_2 = i.color.w;
				if ((i.color.w < 1.0)) {
					maskAlpha_3 = (((maskAlpha_3 - 0.5) * (_MaskConstrast + 1.0)) + 0.5);
					maskAlpha_3 = (maskAlpha_3 * _MaskIntensity);
					float tmpvar_26 = (1.0 - clamp (pow ((1.0 - i.color.w), (8.0 * i.lpos_w.w)), 0.0, 1.0));
					alpha_2 = (clamp ((tmpvar_26 * i.xlv0.y), 0.0, 1.0) + (clamp ((maskAlpha_3 * tmpvar_26), 0.0, 1.0) * (1.0 - i.xlv0.y)));
				};
