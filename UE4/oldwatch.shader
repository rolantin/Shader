...
//--------------------------
float3 GIData(float3 lightColor,float3 lightDirection,float3 normalDirection , float3 posWorld,
...
//light calculations
   ...
   // diffuseColor *= (f0 );
    ...
    float3 DebugColor= float3(1,1,1);
     ...
     float3 lightingModel = (((diffuseColor + saturate(specularity)) + ( (1-_Metallic) * unityIndirectSpecularity )))* attenColor;
  //return  float4    (lightingModel,1);
     lightingModel *= NdotL;
     float4 finalDiffuse = float4(lightingModel  + IBLMap * diffuseColor,1);
